...
from wordwield.lib.db           import RagDocument
from wordwield.lib.fs           import Directory, File
from wordwield.services.rag     import Rag


class Expertise(Service):
	def initialize(self, folder=None):
		if folder is None:
			folder = ww.config.EXPERTISE_DIR

		self.folder       = os.path.abspath(os.path.expanduser(folder))
		self.directory    = Directory(self.folder)
		self.rag          = Rag()
		self.domain       = 'expertise'
		self.readable_ext = ww.config.get('EXPERTISE_FILE_EXT', {'txt', 'md', 'docx', 'html', 'htm', 'pdf'})
