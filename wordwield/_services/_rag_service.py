...
class RagService(Service):
	...
	def _rerank(self, query, texts, reranker):
		if reranker and texts:
			print(f'Reranking with `{reranker}` ...', end=' ', flush=True)
	...
	# ----------------------------------------------------------------------
	def search(self, domain, query, keys=None, k=5, margin=2, ap_prev=None, karma=0.5, reranker=None):
		texts      = []
		ap_next    = ap_prev
		record_ids = self._get_ids_by_keys_and_domain(domain, keys)
		...
		if record_ids:
			...
			texts = self._rerank(query, texts, reranker)[:k]
