...
class WordWield(metaclass=WordWieldMeta):
	...
	def _register_builtins(cls):
		...
		cls.operators['Agent'] = ClassRegistryItem(Agent, {'origin': 'wordwield'})
	...
	@classmethod
	def log_success(cls, msg, end='\n'):
		if cls.verbose:
			print(String.color(f'SUCCESS:', String.GREEN, 'b'), msg, flush=True, end=end)

	@classmethod
	def log_info(cls, msg, end='\n'):
		if cls.verbose:
			print(String.color(f'INFO:   ', String.CYAN, 'b'), msg, flush=True, end=end)

	@classmethod
	def log_warning(cls, msg, end='\n'):
		print(String.color(f'WARNING:', String.YELLOW, 'b'), msg, flush=True, end=end)

	@classmethod
	def log_error(cls, msg, end='\n'):
